...
const ALLOWED_COLUMNS = {
  ...
  scheduled: new Set(['body', 'scheduled_at', 'recurrence_type', 'recurrence_rule',
                      ...
                      'media_type', 'media_path', 'mentions_json', 'media_paths_json']),
...
function registerIpcHandlers(db, waManager, scheduler, notificationManager) {
  ...
  ipcMain.handle('scheduled:create', (_, data) => {
    const result = db.prepare(`INSERT INTO scheduled_messages (account_id, target_type, target_id, target_name, body, media_type, media_path, scheduled_at, recurrence_type, recurrence_rule, next_send_at, mentions_json, media_paths_json)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(data.account_id, data.target_type, data.target_id, data.target_name || '', data.body || '', data.media_type || 'text',
        data.media_path || null, data.scheduled_at, data.recurrence_type || 'once', data.recurrence_rule || null, data.next_send_at || data.scheduled_at, data.mentions_json || null, data.media_paths_json || null)
  ...
  ipcMain.handle('media:pickFiles', async (event) => {
    const win = BrowserWindow.fromWebContents(event.sender)
    const result = await dialog.showOpenDialog(win, {
      title: 'Seleziona file da allegare',
      defaultPath: app.getPath('home'),
